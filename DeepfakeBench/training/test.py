...
- Load detector yaml + optional test_config overrides
- Build test dataloader(s)
- Load pretrained weights (.pth)
- Run inference and collect prob/label/feat
- Compute metrics via get_test_metrics
- Save feature pickle only if --save_feat is enabled
...
parser.add_argument(
    "--detector_path",
    type=str,
    default="training/config/detector/effort_asy.yaml",
...
def prepare_testing_data(config):
    """
    Build a test dataloader for each requested dataset.
    """
...
def main():
    ...
    if "model_name" not in config:
        raise ValueError("YAML must contain config['model_name'] (e.g., effort_asy).")
